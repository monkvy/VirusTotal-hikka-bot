# VirusTotal Hikka Bot

Автор: @monkvy  
Версия: 1.0.0

## Команды

`.vt` — просканировать файл (ответом)  
`.vtl <ссылка>` — просканировать ссылку  
`.vthash <хеш>` — проверить по хешу (SHA256/MD5)  
`.vthistory` — история сканирований  
`.vtclear` — очистить историю  
`.vtlang ru/en` — сменить язык

## Как понять результаты

🚫 Malicious — опасно  
⚠️ Suspicious — подозрительно  
✅ Harmless — безопасно  
👁️ Undetected — не обнаружено

0-2 обнаружено — низкий риск  
3-5 — средний  
6-10 — высокий  
10+ — критический

## Обновление v2.0.0

Премиум эмодзи во всех статусах (только для Premium), Статусы теперь жирным текстом, Убран прогресс-бар (чистые статусы, нет спама в логах), Исправлены эмодзи в инлайн кнопках, Оптимизация и чистка кода

## Получение API ключа

1. Зарегистрируйтесь на [virustotal.com] (ht

2. Перейдите в настройки профиля

3. Скопируйте АРІ ключ

4. Вставьте в конфиг модуля
