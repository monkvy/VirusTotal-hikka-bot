# VirusTotal Hikka Bot

Автор: @monkvy  
Версия: 1.0.0

## Команды

`.vt` — просканировать файл (ответом)  
`.vtl <ссылка>` — просканировать ссылку  
`.vthash <хеш>` — проверить по хешу (SHA256/MD5)  
`.vthistory` — история сканирований  
`.vtclear` — очистить историю  
`.vtlang ru/en` — сменить язык

## Как понять результаты

🚫 Malicious — опасно  
⚠️ Suspicious — подозрительно  
✅ Harmless — безопасно  
👁️ Undetected — не обнаружено

0-2 обнаружено — низкий риск  
3-5 — средний  
6-10 — высокий  
10+ — критический

## Обновление изменений:

• Премиум эмодзи во всех статусах (работает только с премиум аккаунтом, в инлайн кнопках не работают)
• Статусы теперь жирным текстом
• Убрал прогресс-бар (только чистые статусы, проблема была с большими логами в чатах)
• Исправлена проблема с эмодзи в кнопках
• Оптимизация кода
